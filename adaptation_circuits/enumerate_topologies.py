...
def filter_topology_with_path(topology, source, destination):
    """
    Check if a given topology has a direct or indirect path between source and destination nodes.

    Parameters:
    topology (str): The topology string.
    source (str): The source node name.
    destination (str): The destination node name.

    Returns:
    bool: True if there is a path between the source and destination nodes, otherwise False.
    """
    graph = nx.DiGraph()
    ...
    if source in graph and destination in graph:
        ...
        return nx.has_path(graph, source, destination)
...
def build_param_idx_table(nodes, rg, n_samples=1e4, top=None):
    if top is None:
        valid_topologies = enumerate_connected_networks(nodes)
    else:
        valid_topologies = top
...
def count_unique_uppercase_letters(input_str):
    ...
    for char in input_str:
        if char.isupper():
            ...
            numbers.add(ord(char) - ord('A'))
    # todo: convert each uppercase letter to a number
    # Return the number of unique uppercase letters
    return len(unique_uppercase), list(numbers)


def convert_to_sequential(interaction_arr, components_as_num, max_components):
    """convert a list of numbers to a sequential list of numbers. then convert arrays of numbers to that order"""
    components_as_num[-1] = max_components-1 if components_as_num[-1]==14 else components_as_num[-1] #output_node
    conversion_dict = {num: index for index, num in enumerate(sorted(components_as_num))}
    # reversed_dict = {value: key for key, value in conversion_dict.items()}
    # print(reversed_dict)
    if len(interaction_arr) == 0:
        return np.array([])
    else:
        converted_list = [[conversion_dict[i] for i in row] for row in interaction_arr]
...
def topologies_to_num_params(topologies):
    ...
    num_params = {}
    for topology in topologies:
        num_params[topology], _ = count_unique_uppercase_letters(topology)
    return num_params
